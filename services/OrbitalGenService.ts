...
const enforceTwoToOneAspect = (dataUrl: string): Promise<void> =>
  new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      ...
      if (Math.abs(ratio - 2) > 0.02) {
        reject(
          new Error(
            `INVALID_ASPECT_RATIO: expected 2:1 canvas, received ${img.width}x${img.height}`,
          ),
        );
      } else {
        resolve();
    ...
    img.onerror = () => reject(new Error("INVALID_IMAGE_PAYLOAD"));
    img.src = dataUrl;
  });
...
const generateRing = async (pitchAngle: number): Promise<string> => {
    ...
    const response: GenerateContentResponse = await ai.models.generateContent({
      ...
      config: {
        imageConfig: { aspectRatio: "2:1", imageSize: options.imageSize || "1K" }
      }
    });
    ...
    await enforceTwoToOneAspect(imageUrl);
