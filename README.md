...
4. (Optional) In **Settings → Secrets and variables → Actions**, add `GEMINI_API_KEY` and `VITE_GEMINI_API_KEY`. The deploy workflow detects their presence for logging but explicitly clears them during the static `npm run build` step so they are not bundled. Users still enter the key at runtime via the UI/session storage.
